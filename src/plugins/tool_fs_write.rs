...
impl Tool for ToolFsWrite {
    ...
    async fn execute(&self, input: ToolInput, context: ToolContext) -> Result<ToolOutput> {
        let path = input
            ...
            .ok_or_else(|| Error::Tool("fs_write requires a non-empty path".into()))?;
        let content = input
            .get("content")
            .and_then(Value::as_str)
            .ok_or_else(|| Error::Tool("fs_write requires content".into()))?;
        ...
        add_output_note(&context, "fs_write", format!("Wrote {path}"), &output).await?;
        Ok(output)
    }
}

pub fn parse_fs_write_freeform(input: &str) -> Result<Value> {
    let (header, body) = input.split_once('\n').ok_or_else(|| {
        Error::Tool("fs_write freeform input requires a header and content".into())
    })?;
    let header = header.strip_suffix('\r').unwrap_or(header);
    let specification = header
        .strip_prefix("WRITE ")
        .ok_or_else(|| Error::Tool("fs_write header must start with `WRITE `".into()))?;
    let (path, tag) = specification
        .rsplit_once(" <<<")
        .ok_or_else(|| Error::Tool("fs_write header must end with `<<<TAG`".into()))?;
    if path.trim().is_empty() || tag.is_empty() || tag.chars().any(char::is_whitespace) {
        return Err(Error::Tool(
            "fs_write header has an invalid path or tag".into(),
        ));
    }
    let closing = format!("\n{tag}");
    let content = body
        ...
        .ok_or_else(|| Error::Tool("fs_write content is missing its closing tag".into()))?;
    Ok(json!({ "path": path.trim(), "content": content }))
}

pub struct ToolFsWritePlugin {
    id: PluginId,
    tool: Arc<ToolFsWrite>,
}

impl ToolFsWritePlugin {
    pub fn new() -> Self {
        Self {
            id: PluginId::new(),
            tool: Arc::new(ToolFsWrite::new()),
        }
    }
}

#[async_trait]
impl Plugin for ToolFsWritePlugin {
    fn id(&self) -> PluginId {
        self.id
    }
